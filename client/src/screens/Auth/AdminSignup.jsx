...
let AdminSignup = () => {
    ...
    let submitHandler = useCallback(async(e) =>{
        e.preventDefault()
        if(username && userPhoneNumber && userEmail &&userPassword && userConfirmPassword && userState && userCountry){
            //submitt form 
            setIsLoading(true)

            let res = await dispatch(adminsignup({
                username,
                userPhoneNumber,
                userEmail,
                userPassword,
    ...
    },[adminsignup,username,
        userPhoneNumber,
        userEmail,
        userPassword,
        userConfirmPassword,
        userState,
        userCountry,adminsignup,navigate])
    ...
    return (<>
        ...
        <Formheader className={styles.togglemenu} title="login" link='adminlogin'/>
